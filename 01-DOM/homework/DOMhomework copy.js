...
function buildToDo(todo, index) {
  ...
  //toDoShell.classList.add => para agregar algo mas a la clase
  //paso 3
...
function buildToDos(toDos) {
  // Tu código acá:
  let newArr = toDos.map( (element, i) => {
    return buildToDo(element, i);
  })
...
function displayToDos() {
  ...
  //paso 4
  result.map (t => {
    return toDoContainer.appendChild(t);
  })
}
